...
- **Camera framing before a shot.** Two Studio commands put the viewport where you want it before `screen_capture`: `adjust_camera {target?, axis?, distance?, elevation?, fov?, direction?}` frames a part, a model, an instance path, `x,y,z` coordinates, or whatever is **selected in Studio** — with no target it keeps the aim and just moves closer/further, and with no `axis` it keeps your current viewing angle (it reframes, it does not teleport the view). `set_camera_axis {axis, target?, distance?, elevation?, fov?}` is the same engine with the axis **required**: `+x -x +y -y +z -z` or `front back left right top bottom`. Distance defaults to auto-framing from the subject's bounding box and the FOV, `elevation` (default 15°) lifts the camera above the horizon, and a vertical axis gets a safe up vector. So "the camera is facing away from the dog" is one call away. (`camera_look_at` from the daily set still works.)
- **Capture speed.** The wait you feel is the *chat upload* of the picture, not the capture itself, and a full-viewport Studio PNG is often several MB. Captures over 350 KB are therefore resized to at most 1400 px on the long side and re-encoded as JPEG q0.9 — but only when that actually produces fewer bytes, and small captures keep their exact original PNG bytes. Blender's hand-off PNG is no longer awaited before the picture is returned (it is deleted in the background), which removes a whole round trip per Blender capture. **Settings → Captures → "Fast screenshots"** toggles this on/off (off = full-size originals); it applies to the very next capture with no reload. Power users can still set it directly in the service-worker console: `chrome.storage.local.set({"rs-shot-max": 1024, "rs-shot-quality": 0.85})`, or `{"rs-shot-max": 0}` for originals.
