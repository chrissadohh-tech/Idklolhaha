...
- **Persistent Visual Reference in Settings.** Drag and drop any reference image directly into **Settings → Visual Reference** and designate its target: **GUI Layout** or **Build / 3D Model**.
  ...
  - **Iterative screenshot comparison:** Whenever the AI creates or modifies the GUI or build in Studio, it captures the viewport with `screen_capture`. The extension delivers the Studio capture side-by-side with your reference image, prompting the model to calculate discrepancies, adjust UDim2 or CFrame coordinates, and refine in a loop until the result is identical or identically close.
