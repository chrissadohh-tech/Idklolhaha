...
-- ═════════════════════════════════════════════════════════════════════════════
	...
	-- ── Viewport camera framing (prepare a shot instead of guessing) ──────────
-- During Edit the studio viewport camera IS workspace.CurrentCamera, so moving
-- it here is exactly what the next screen_capture will show. Two entry points,
-- one engine: adjust_camera (axis optional) and set_camera_axis (axis required).
local AXIS_VECTORS = {
	["+x"] = Vector3.new(1, 0, 0), x = Vector3.new(1, 0, 0), right = Vector3.new(1, 0, 0), east = Vector3.new(1, 0, 0),
	["-x"] = Vector3.new(-1, 0, 0), left = Vector3.new(-1, 0, 0), west = Vector3.new(-1, 0, 0),
	["+y"] = Vector3.new(0, 1, 0), y = Vector3.new(0, 1, 0), top = Vector3.new(0, 1, 0), above = Vector3.new(0, 1, 0),
	["-y"] = Vector3.new(0, -1, 0), bottom = Vector3.new(0, -1, 0), below = Vector3.new(0, -1, 0),
	["+z"] = Vector3.new(0, 0, 1), z = Vector3.new(0, 0, 1), back = Vector3.new(0, 0, 1), behind = Vector3.new(0, 0, 1),
	["-z"] = Vector3.new(0, 0, -1), front = Vector3.new(0, 0, -1),
}

local function vec3Of(v)
	if type(v) == "table" then
		local x = tonumber(v[1] or v.x or v.X)
		local y = tonumber(v[2] or v.y or v.Y)
		local z = tonumber(v[3] or v.z or v.Z)
...
local function selectionList()
	local okS, sel = pcall(function() return game:GetService("Selection"):Get() end)
	if okS and sel and #sel > 0 then return sel end
	return nil
end

-- "Dog" / "workspace.Dog.Model" / an Instance / the current Studio selection.
local function resolveInstance(target)
	if type(target) == "Instance" then return target end
	if target == nil or tostring(target) == "" then return nil end
	local s = tostring(target)
	local inst = resolvePath(s)
	if inst then return inst end
	local sel = selectionList()
	if sel and sel[1].Name == s then return sel[1] end
	local scopes = { workspace, game:GetService("ReplicatedStorage") }
	for _, src in ipairs({ "ServerStorage", "StarterGui", "StarterPlayer", "Lighting", "ServerScriptService" }) do
		local ok2, svc = pcall(function() return game:GetService(src) end)
		if ok2 and svc then scopes[#scopes + 1] = svc end
	end
	for _, scope in ipairs(scopes) do
		local found = scope:FindFirstChild(s, true)
		if found then return found end
	end
	return nil
end

-- Centre + radius of whatever we were pointed at (part, model, folder, position).
local function subjectOf(target)
	if target == nil or tostring(target) == "" then
		local sel = selectionList()
		if not sel then return nil end
		target = sel[1]
	end
	local v = vec3Of(target)
	if v then return v, 0.5, "position" end
	local inst = resolveInstance(target)
	if not inst then return nil end
	local label = inst.Name .. " (" .. inst.ClassName .. ")"
	if inst:IsA("Model") then
		local okB, cf, size = pcall(function() return inst:GetBoundingBox() end)
		if okB and cf and size then
			return cf.Position, math.max(size.X, size.Y, size.Z) * 0.5, label
		end
	end
	if inst:IsA("BasePart") then
		return inst.Position, math.max(inst.Size.X, inst.Size.Y, inst.Size.Z) * 0.5, label
	end
	if inst:IsA("Attachment") then
		return inst.WorldPosition, 0.5, label
	end
	-- Folder / Script / anything else: union of the parts inside it.
	local lo, hi = nil, nil
	local function include(cf, size)
		local half = size * 0.5
		local a = cf.Position - half
		local b = cf.Position + half
	...
	local okD, desc = pcall(function() return inst:GetDescendants() end)
	if okD then
		for _, d in ipairs(desc) do
			if d:IsA("BasePart") then
				local okS, cf, size = pcall(function() return d.CFrame, d.Size end)
				if okS and cf and size then include(cf, size) end
			end
		end
	end
	if lo and hi then
		local size = hi - lo
		return (lo + hi) * 0.5, math.max(size.X, size.Y, size.Z) * 0.5, label
	end
	local okP, pivot = pcall(function() return inst:GetPivot() end)
	if okP and pivot then return pivot.Position, 0.5, label end
	return nil
end

local function frameCamera(a, axisRequired)
	local cam = workspace.CurrentCamera
	if not cam then cam = workspace:FindFirstChildOfClass("Camera") end
	if not cam or not cam:IsA("Camera") then
		return fail("no viewport camera in this datamodel - open a place in Studio")
	end

	local focus, radius, subjectDesc = subjectOf(a.target)
	if not focus then
		-- Nothing named and nothing selected: keep the aim, just move along the
		-- current view direction (useful for 'get closer / further').
		local d = tonumber(a.distance) or 30
		focus = cam.CFrame.Position + cam.CFrame.LookVector * d
		radius = 0.5
		subjectDesc = "current view"
	end
	if radius < 0.5 then radius = 0.5 end

	local fov = cam.FieldOfView or 70
	if tonumber(a.fov) then fov = math.clamp(tonumber(a.fov), 20, 120) end

	local axisName = a.axis or a.face or a.view
	if axisRequired and (axisName == nil or tostring(axisName) == "") then
		return fail("set_camera_axis needs axis = +x, -x, +y, -y, +z, -z (or front, back, left, right, top, bottom)")
	end
	local hasAxis = axisName ~= nil and tostring(axisName) ~= ""

	local dir = nil
	if hasAxis then
		dir = AXIS_VECTORS[tostring(axisName):lower()]
		if not dir then
			return fail("unknown axis '" .. tostring(axisName) .. "' - use +x, -x, +y, -y, +z, -z or front/back/left/right/top/bottom")
		end
	else
		dir = vec3Of(a.direction)
		if dir and dir.Magnitude > 0.01 then dir = dir.Unit else dir = nil end
	end

	-- Elevation: lift the camera above the horizon (a pure vertical axis keeps its
	-- direction and only gets a hair of tilt, so lookAt is never degenerate).
	if dir then
		local flat = Vector3.new(dir.X, 0, dir.Z)
		if flat.Magnitude > 0.05 then
			local elev = math.rad(tonumber(a.elevation) or 15)
			dir = (flat.Unit * math.cos(elev) + Vector3.new(0, math.sin(elev), 0)).Unit
		else
			dir = (dir + Vector3.new(0, 0, dir.Y > 0 and -0.001 or 0.001)).Unit
		end
	end
	if not dir then
		-- No axis: keep the CURRENT bearing around the subject, so "look at the
		-- dog" reframes instead of teleporting the view to another side.
		local rel = cam.CFrame.Position - focus
		local flat = Vector3.new(rel.X, 0, rel.Z)
		if flat.Magnitude < 0.05 then flat = Vector3.new(0, 0, 1) else flat = flat.Unit end
		local elev = math.rad(tonumber(a.elevation) or 15)
		dir = (flat * math.cos(elev) + Vector3.new(0, math.sin(elev), 0)).Unit
	end

	local dist = tonumber(a.distance)
	if not dist or dist <= 0 then
		-- Auto-frame: the subject's radius must fit the vertical FOV, with margin
		-- so the model is not cropped edge to edge.
		dist = (radius / math.tan(math.rad(fov) * 0.5)) * 1.7
	end
	dist = math.clamp(dist, 2, 5000)

	local eye = focus + dir * dist
	if (eye - focus).Magnitude < 0.05 then
		return fail("that camera position would sit on the subject - pass a larger distance")
	end
	local up = Vector3.new(0, 1, 0)
	if math.abs(dir.Y) > 0.98 then
		up = Vector3.new(0, 0, dir.Y > 0 and -1 or 1)
	end

	pcall(function() cam.Focus = focus end)
	cam.CFrame = CFrame.lookAt(eye, focus, up)
	if tonumber(a.fov) then cam.FieldOfView = fov end

	local view = hasAxis and tostring(axisName):lower() or "kept the current angle"
	local text = string.format(
		"Camera at (%.1f, %.1f, %.1f) looking at (%.1f, %.1f, %.1f) - %s, %.1f studs away, FOV %.0f. Subject: %s.",
		eye.X, eye.Y, eye.Z, focus.X, focus.Y, focus.Z, view, dist, cam.FieldOfView, tostring(subjectDesc))
	return okdata({
		camera = { eye.X, eye.Y, eye.Z },
		focus = { focus.X, focus.Y, focus.Z },
		distance = dist,
		axis = view,
		subject = tostring(subjectDesc),
		field_of_view = cam.FieldOfView,
	}, text .. " Now take the screenshot (screen_capture).")
end

function api.adjust_camera(a)
	return frameCamera(a or {}, false)
end

function api.set_camera_axis(a)
	return frameCamera(a or {}, true)
end
...
local dispatch = {
	...
	adjust_camera = api.adjust_camera,
	set_camera_axis = api.set_camera_axis,
...
const SKILL_COMMANDS = [
  ...
  {
    name: "adjust_camera",
    description: "Aim Studio's viewport camera at something and frame it for a screenshot - a part/model name, an instance path, or 'x,y,z' coordinates. With no target it frames the Studio selection, and with nothing selected it keeps the aim and just moves closer/further. Keeps the current viewing angle unless you pass axis.",
    params: {
      target: { type: "string", req: false, desc: "e.g. 'Dog', 'workspace.Dog.Body', '10,4,0' - default: Studio selection" },
      axis: { type: "string", req: false, desc: "+x, -x, +y, -y, +z, -z or front, back, left, right, top, bottom (default: keep current angle)" },
      distance: { type: "number", req: false, desc: "studs from the subject (default: auto-frames it)" },
      elevation: { type: "number", req: false, desc: "degrees above the horizon, default 15" },
      fov: { type: "number", req: false, desc: "FieldOfView 20-120 (default: keep current)" },
      direction: { type: "string", req: false, desc: "explicit direction instead of axis, e.g. '0,1,-1'" }
    }
  },
  {
    name: "set_camera_axis",
    description: "Put the viewport camera on a world axis looking at the subject - the same framing engine as adjust_camera, with the axis required. Use it for predictable shots: front/back/left/right/top/bottom or +x/-x/+y/-y/+z/-z.",
    params: {
      axis: { type: "string", req: true, desc: "+x, -x, +y, -y, +z, -z or front, back, left, right, top, bottom" },
      target: { type: "string", req: false, desc: "subject name/path or 'x,y,z' - default: Studio selection" },
      distance: { type: "number", req: false, desc: "studs away (default: auto-frames)" },
      elevation: { type: "number", req: false, desc: "degrees above the horizon (0 = dead-on side view)" },
      fov: { type: "number", req: false, desc: "FieldOfView 20-120" }
    }
  },
  {
